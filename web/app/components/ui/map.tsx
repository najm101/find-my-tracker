...
// `?worker&url` bundles the worker with the shared chunk it imports; plain `?url` would not.
import workerUrl from "maplibre-gl/dist/maplibre-gl-worker.mjs?worker&url"

import { cn } from "~/lib/utils"

// Find My Tracker: serve the worker from our own build instead of unpkg, so the app loads no
// third-party code at runtime.
if (typeof window !== "undefined" && !MapLibreGL.getWorkerUrl()) {
  MapLibreGL.setWorkerUrl(workerUrl)
}

// Find My Tracker: OpenFreeMap (free, no API key, OpenStreetMap data) instead of CARTO, whose
// basemaps need a license for commercial use.
const defaultStyles = {
